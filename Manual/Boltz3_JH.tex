...
\author{Andre Lessa, this version edited by JH}
\abstract{}
...
\end{equation}
\com{JH: afaics $F=g f$, where $f$ is the phase space density in Kolb/Turner and $g$ the d.o.f.
and $C = C_{\mathrm{Kolb}} b/E$.}
where $F_{i}(p,t)$ is the number distribution of particle $i$ as function of momentum $p=|\vec p|$, $C$ represents a source/sink term and $H$ is the Hubble parameter:
\begin{equation}
...
of $F_{i}$ as:
\begin{eqnarray}
n_{i}(t) & = & \int \frac{dp}{2 \pi^2} p^2 F_i(p,t) \nonumber \\ 
\rho_{i}(t) & = & \int \frac{dp}{2 \pi^2} p^2 E_i F_i(p,t) \label{beqs}\\
P_{i}(t) & = & \frac{1}{3} \int \frac{dp}{2 \pi^2} \frac{p^4}{E_i} F_i(p,t) \nonumber
\end{eqnarray}
...
\end{eqnarray}
If $CP$ is conserved, the collision term, $C_i$, for the process $i + j +k+ \ldots \leftrightarrow a
+ b +  \ldots$ is given by~\cite{kawasaki0}:
\begin{eqnarray}
C_i & = & \frac{1}{E_i} \int \prod_{\{j,k,\ldots\}}  \frac{d^3 p_j}{2 E_j (2 \pi)^3} \prod_{\{a,b,\ldots\}}
